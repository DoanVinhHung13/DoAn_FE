...
const getMaterialTypeLabel = (material, fallback = "OTHER") => {
  ...
  return (
    MATERIAL_TYPE_LABELS[normalizedType] || type || MATERIAL_TYPE_LABELS.OTHER
  )
}
...
const getMaterialUnit = (material, fallback = "") => {
  const explicitUnit =
    material?.unit || material?.quantityUnit || material?.measurementUnit
  ...
  return (
    ...
    (type === "FERTILIZER"
      ? "kg"
      : type === "PESTICIDE" || type === "CROP_PROTECTION"
        ? "lít"
        : "")
  )
}
...
const formatMaterialQuantity = material => {
  ...
  if (typeof quantity === "string" && /[a-zA-ZÀ-ỹ]/.test(quantity))
    return quantity
...
const getMaterialColorClass = material => {
  ...
  if (
    ...
    type.includes("THU HOẠCH") ||
    type.includes("SẢN LƯỢNG")
  ) {
    return "text-emerald-700"
  }
  if (type.includes("FERTILIZER") || type.includes("PHÂN BÓN"))
    return "text-blue-700"
  if (
    type.includes("PESTICIDE") ||
    type.includes("CROP_PROTECTION") ||
    type.includes("NÔNG DƯỢC")
  ) {
...
const buildTimelineGroups = traceData => {
  ...
  const officialLogs = Array.isArray(traceData.cultivationLogs)
    ? traceData.cultivationLogs
    : []
  const dailyLogs = Array.isArray(traceData.dailyLogs)
    ? traceData.dailyLogs
    : []
        ...
        const dailyLog =
          dailyLogs.find(
            item =>
              item?.date &&
              log?.activityDate &&
              parseDate(item.date).isSame(parseDate(log.activityDate), "day"),
          ) || dailyLogs[index]
          ...
          materials
            .map(material => {
              ...
              return quantity == null
                ? name
                : `${name}: ${quantity} ${unit}`.trim()
            })
            .join("; ")

        return {
          stage:
            log.cultivationStageName ||
            dailyLog?.stage ||
            log.stage ||
            "Giai đoạn canh tác",
  ...
  return rawEntries.reduce((groups, entry) => {
    let group = groups.find(
      item =>
        item.stage === entry.stage && item.stageOrder === entry.stageOrder,
    )
    if (!group) {
      group = {
        stage: entry.stage,
    ...
    if (
      start?.isValid() &&
      (!group.startDate || start.isBefore(parseDate(group.startDate), "day"))
    ) {
      group.startDate = entry.startDate
    }
    if (
      end?.isValid() &&
      (!group.endDate || end.isAfter(parseDate(group.endDate), "day"))
    ) {
...
export const TraceView = ({ traceabilityData, qrCode, isPreview = false }) => {
  const traceData = useMemo(() => {
    ...
    const logMaterials = cultivationLogs.flatMap(log => {
      ...
      const fertilizerMaterials = toArray(
        log?.totalFertilizers || log?.fertilizers,
      ).map(material =>
        normalizeMaterial(
          material,
          "FERTILIZER",
      ...
      const pesticideMaterials = toArray(
        log?.totalPesticides || log?.pesticides,
      ).map(material =>
        normalizeMaterial(
          material,
          "PESTICIDE",
    ...
    const journalMaterials = (
      logMaterials.length ? logMaterials : b.materials
    ).map(material => normalizeMaterial(material))
    const logPhotos = cultivationLogs
      .flatMap(log =>
    ...
    const journalAreas = cultivationLogs
      .map(log => {
        const materials = Array.isArray(log?.materials) ? log.materials : []
        ...
        return {
          area:
            executedArea > 0
              ? executedArea
              : materialAreas.length
                ? Math.max(...materialAreas)
                : 0,
          areaUnit:
            materials.find(material => material?.areaUnit)?.areaUnit ||
            log?.areaUnit ||
            "",
        }
      })
    ...
    const areaUnit = formatAreaUnit(
      source?.areaUnit || payload?.areaUnit || journalAreaUnit,
    )
    return {
      ...
      area:
        areaValue != null
          ? [formatAreaValue(areaValue), areaUnit].filter(Boolean).join(" ")
          : "—",
          ...
          <span className="text-xs text-amber-700 font-medium">
            ...
            {traceabilityData?.verificationStatus && (
              <>
                {" "}
                Trạng thái:{" "}
                <strong>{traceabilityData.verificationStatus}</strong>.
              </>
            )}
          </span>
        </div>
      ...
      <div className="border-b border-emerald-100 bg-[#f3f9f5]">
        <div className="mx-auto max-w-3xl px-4 py-6 sm:px-6 sm:py-8">
          <div className="flex flex-col items-center gap-4 text-center sm:flex-row sm:items-center sm:text-left">
            ...
            <div className="flex-1 min-w-0">
              <div className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700 mb-2">
                <CheckCircleOutlined className="text-emerald-300" /> Hệ thống
                truy xuất nguồn gốc canh tác nông sản
              </div>
              <Title
                level={2}
                className="!text-emerald-800 !mb-1 text-xl sm:text-2xl font-bold tracking-tight"
              >
                Truy xuất nguồn gốc
              </Title>
              <Text className="text-slate-500 text-xs sm:text-sm font-mono block">
                Mã QR:{" "}
                <strong className="text-white bg-black/20 px-2 py-0.5 rounded">
                  {traceData.qrCode}
                </strong>
              </Text>
            </div>
          ...
          <div className="mt-4 flex items-start gap-3 rounded-2xl border border-emerald-100 bg-white p-4 text-xs shadow-sm sm:text-sm">
            ...
            <span>
              Sản phẩm được theo dõi và xác thực 100% dữ liệu điện tử từ quy
              trình gieo trồng đến thu hoạch.
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-3xl space-y-5 px-4 pt-5 sm:px-6 sm:pt-6">
        ...
        <Card className="trace-card overflow-hidden rounded-2xl border border-slate-200/80 shadow-sm">
          <div className="flex items-center gap-2 mb-4 pb-3 border-b border-slate-100">
            ...
            <Title
              level={4}
              className="!mt-0 !mb-0 !text-base sm:!text-lg font-bold text-slate-800"
            >
          ...
          <div className="grid grid-cols-1 gap-4 text-xs sm:grid-cols-2 sm:text-sm">
            <div className="p-3 bg-slate-50/80 rounded-xl border border-slate-200/70 flex flex-col justify-between">
              <Text className="text-slate-500 text-xs font-semibold block mb-1">
                Mã lô sản xuất
              </Text>
              <div>
                <Tag
                  color="blue"
                  className="text-xs sm:text-sm font-semibold px-2.5 py-0.5 rounded-md m-0"
                >
                  {traceData.batchCode}
                </Tag>
              </div>
            ...
            <div className="p-3 bg-slate-50/80 rounded-xl border border-slate-200/70 flex flex-col justify-between">
              ...
              <Text strong className="text-sm sm:text-base text-emerald-800">
                {traceData.cropName}
              </Text>
            </div>
              ...
              <Space className="text-xs sm:text-sm">
                ...
                <Text strong className="text-blue-700">
                  {traceData.harvestDate
                    ? formatDate(traceData.harvestDate)
                    : "—"}
                </Text>
              </Space>
            </div>
            ...
            <div className="p-3 bg-slate-50/80 rounded-xl border border-slate-200/70 flex flex-col justify-between">
              ...
              <Text className="text-xs sm:text-sm font-semibold text-slate-800">
                {traceData.area} •{" "}
                <span className="text-emerald-700">{traceData.yield}</span>
              </Text>
            </div>
          </div>
        </Card>
        ...
        {traceData.displayOptions.showDailyLog && (
          <Card className="trace-card rounded-2xl border border-slate-200/80 shadow-sm">
            <div className="flex items-center gap-2 mb-4 pb-3 border-b border-slate-100">
              ...
              <Title
                level={4}
                className="!mb-0 !text-base sm:!text-lg font-bold text-slate-800"
              >
            ...
            {timelineGroups.length > 0 ? (
              <div className="space-y-9 px-1 sm:px-2">
                {timelineGroups.map(group => (
                  <section
                    key={`${group.stage}-${group.stageOrder ?? "unknown"}`}
                  >
                    <div className="mb-6">
                      <Text
                        strong
                        className="block text-base font-bold leading-6 text-slate-800 sm:text-lg"
                      >
                        {group.stage}
                      </Text>
                    ...
                    <div className="relative ml-2 border-l-2 border-emerald-300 pl-5 sm:pl-6">
                      {group.entries.map((entry, index) => (
                        <article
                          key={`${entry.taskName}-${entry.startDate}-${index}`}
                        ...
                        >
                          ...
                          <Text
                            strong
                            className="block text-sm font-bold leading-6 text-slate-800 sm:text-base"
                          >
                            {entry.taskName}
                          </Text>
                          <Text
                            strong
                            className="mt-1 block text-sm leading-6 text-slate-800"
                          >
                              ...
                              <Image.PreviewGroup items={entry.images}>
                                <div className="mt-3">
                                  <Text className="mb-1.5 flex items-center gap-1 text-xs font-semibold text-slate-600">
                                    <FileImageOutlined /> Ảnh minh chứng (
                                    {entry.images.length})
                                  </Text>
        ...
        {(traceData.displayOptions.showMaterials ??
            ...
            <Card className="trace-card rounded-2xl border border-slate-200/80 shadow-sm">
              <div className="mb-4 flex items-start gap-3 border-b border-slate-100 pb-4">
                ...
                <div>
                  <Title
                    level={4}
                    className="!mb-0.5 !text-base sm:!text-lg font-bold text-slate-900"
                  >
                    Phân bón và nông dược
                  </Title>
                  <Text className="text-xs text-slate-500">
                    Các loại đã sử dụng trong nhật ký này
                  </Text>
                </div>
              </div>
                ...
                <table className="w-full min-w-[560px] border-collapse text-left text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100 bg-white">
                    {traceData.materials.map((material, index) => (
                      <tr
                        key={`table-${material.id || `${material.name}-${material.usedAt || index}`}`}
                      >
                        <td className="px-4 py-3">
                          <Tag
                            color={
                              material.type === "Nông dược" ? "purple" : "blue"
                            }
                        ...
                        <td className="px-4 py-3 text-slate-500">
                          {material.usedAt ? formatDate(material.usedAt) : "—"}
                        </td>
                      </tr>
                    ))}
                  </tbody>
              ...
              <div className="trace-material-legacy grid grid-cols-1 gap-3">
                {traceData.materials.map((material, index) => (
                  <div
                    key={
                      material.id ||
                      `${material.name}-${material.usedAt || index}`
                    }
                    className="rounded-xl border border-slate-200/80 bg-slate-50/80 p-3.5 sm:p-4"
                  >
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      ...
                      <div className="shrink-0 rounded-lg border border-emerald-200 bg-emerald-50 px-2.5 py-1.5 sm:text-right">
                        <Text className="block text-[11px] font-medium text-emerald-700">
                          Số lượng đã dùng
                        </Text>
                        <Text strong className="text-base text-emerald-800">
                          {formatMaterialQuantity(material)}
                        </Text>
                      </div>
                    </div>
...
const Trace = () => {
  ...
  useEffect(() => {
    ...
    if (!qrCode || !payload?.isValid || recordedScanCodes.current.has(qrCode))
      return

    recordedScanCodes.current.add(qrCode)
    void http
      .post(`/traceability/${encodeURIComponent(qrCode)}/scan`, null, {
        skipNotice: true,
        skipAuthRedirect: true,
      })
      .catch(() => {
  ...
  return (
    <TraceView traceabilityData={payload} qrCode={qrCode} isPreview={false} />
  )
}
