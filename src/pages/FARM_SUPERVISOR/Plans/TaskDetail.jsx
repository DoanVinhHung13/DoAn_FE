...
const FarmSupervisorTaskDetail = () => {
    ...
    const loadTaskAndUsers = async () => {
      ...
      try {
        const [planRes, leadersRes, farmersRes] = await Promise.all([
          !passedPlanData
            ? CultivationLogbookService.getById(planId).catch(() => null)
            : Promise.resolve({ data: passedPlanData }),
          ...
          if (foundTask) {
            ...
            if (foundTask.assignedLeaderId) {
              assignForm.setFieldsValue({
                ...
                farmerIds:
                  foundTask.assignments?.map(a =>
                    typeof a === "object" ? a.userId || a.id : a,
                  ) || [],
              })
            }
            if (foundTask.leaderSummary) {
              compileForm.setFieldsValue({
                supervisorDescription:
                  foundTask.leaderSummary.description || "",
              })
            }
          } else {
            navigate(
              ROUTER.FS_CULTIVATION_LOGBOOK_DETAIL.replace(":planId", planId),
            )
          }
        }
  ...
  const handleActivate = async () => {
    ...
    if (isHarvestTask(task)) {
      ...
      const unfinishedTasks = allTasks.filter(
        item => !isHarvestTask(item) && item.status !== "COMPLETED",
      )
      const finalStage = [...planStages]
        .filter(item => !item.isDeleted)
        .sort(
          (left, right) => (right.stageOrder || 0) - (left.stageOrder || 0),
        )[0]
      const unfinishedStages = planStages.filter(
        item => item.id !== finalStage?.id && item.status !== "COMPLETED",
      )
    ...
    if (
      isHarvestTask(task) &&
      getActiveQuarantineWarnings(quarantineWarnings).length > 0
    ) {
      message.warning(
  ...
  const fertilizerRows =
    task.leaderSummary?.totalFertilizers ||
    task.leaderSummary?.fertilizers ||
    []
  const pesticideRows =
    task.leaderSummary?.totalPesticides || task.leaderSummary?.pesticides || []
      ...
      <div>
        ...
        {task.description && (
          <div className="mt-3 rounded-xl bg-amber-50 border border-amber-100 p-3 text-sm text-amber-900">
            <InfoCircleOutlined className="mr-2" /> <strong>Hướng dẫn:</strong>{" "}
            {task.description}
          </div>
        )}
              ...
              {["PENDING", "ASSIGNED"].includes(task.status) ? (
                <Form form={assignForm} layout="vertical">
                  <Form.Item
                    ...
                    rules={[
                      { required: true, message: "Chọn người phụ trách" },
                    ]}
                  ...
                  <Button
                    ...
                    loading={activating}
                    className="w-full h-10 mt-2 font-semibold bg-blue-600"
                    ...
                    <div className="flex flex-wrap items-center gap-2 p-2 rounded-lg bg-blue-50">
                      ...
                      <span>
                        ...
                        {task.assignments
                          ? task.assignments
                              .filter(f => !f.isLeader)
                              .map(f => f.fullName || f.name)
                              .join(", ")
                          : task.farmerNames.join(", ")}
                      </span>
                    </div>
                  )}
                  <Alert
            ...
            {task.leaderSummary ? (
              ...
              >
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <Tag color="success" className="ml-auto rounded-full">
                ...
                <Collapse
                  bordered={false}
                  defaultActiveKey={["data", "images", "description"]}
                ...
                >
                          ...
                          >
                            ...
                            <div className="text-xs text-gray-500 mt-1">
                              {f.dailyBreakdown
                                ?.map(
                                  d =>
                                    `${formatDate(d.date)}: ${d.quantity} ${getQuantityUnit(f.quantityUnit || f.unit, MEASUREMENT_UNITS.KILOGRAM)}/${d.area} ${formatAreaUnit(MEASUREMENT_UNITS.SQUARE_METER)}`,
                                )
                                .join(" · ")}
                            </div>
                          </div>
                    ...
                    >
                      <div className="space-y-2">
                        {pesticideRows.map(p => (
                          <div
                            key={p.name}
                            className="rounded-lg bg-purple-50 p-2 text-sm"
                          >
                            <div className="font-semibold">{p.name}</div>
                            <div>
                              Tổng: {p.totalQuantity}{" "}
                              {getQuantityUnit(
                  ...
                  {task.leaderSummary.images?.length > 0 && (
                    <Collapse.Panel
                      header={`📷 Ảnh đính kèm (${task.leaderSummary.images.length})`}
                      key="images"
                    >
                      <Image.PreviewGroup>
                  ...
                  {/* Mô tả từ Leader */}
                  <Collapse.Panel
                    header="✍️ Mô tả tổng kết từ Tổ trưởng"
                    key="description"
                  >
                ...
                {!task.officialLog ? (
                  ...
                  <div className="mt-4 rounded-xl bg-green-50 border border-green-200 p-4">
                    ...
                    <div className="mt-3 text-sm">
                      ...
                      <div className="font-mono bg-white rounded-lg p-2">
                        {task.officialLog.dataSentence}
                      </div>
                      <div className="font-semibold mt-3 mb-1">
                        Mô tả đã biên tập:
                      </div>
                      <div className="italic bg-white rounded-lg p-2">
              ...
              >
                <div className="text-center py-8 text-gray-400">
                  ...
                  <div>
                    Kích hoạt công việc để người phụ trách bắt đầu ghi nhật ký.
                  </div>
                </div>
              </Card>
            ) : null}
      ...
      >
        <Form form={compileForm} layout="vertical" className="space-y-4">
          ...
          {task.leaderSummary?.images?.length > 0 && (
            <div>
              <div className="text-xs font-semibold text-gray-500 mb-2 uppercase tracking-wide">
                📷 Ảnh đính kèm (KHÔNG ĐƯỢC XÓA/SỬA -{" "}
                {task.leaderSummary.images.length} ảnh)
              </div>
              <Image.PreviewGroup>
