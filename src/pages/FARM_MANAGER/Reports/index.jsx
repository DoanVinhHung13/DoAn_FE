...
const readField = (source, keys) => {
  const candidates = [source, source?.summary, source?.Summary, source?.totals, source?.Totals]
...
const formatUnitValue = (value, unit) => `${formatNumber(value)} ${unit || ''}`.trim()
...
const normalizeHarvestReport = (response) => {
  ...
  const rows = sourceRows.map((row, index) => ({
    key: `harvest-${index}`,
    cropName: readField(row, ['CropName', 'cropName', 'Crop', 'crop', 'Name', 'name']) || 'Chưa phân loại',
    value: getRowNumber(row, valueKeys) ?? 0,
    unit: readField(row, ['Unit', 'unit', 'YieldUnit', 'yieldUnit', 'QuantityUnit', 'quantityUnit']) || 'kg',
  }))
  const payloadTotal = toNumber(readField(payload, valueKeys))
  const unit = readField(payload, ['Unit', 'unit', 'YieldUnit', 'yieldUnit', 'QuantityUnit', 'quantityUnit']) || rows[0]?.unit || 'kg'
  const total = payloadTotal ?? sumValues(rows, ['value'])
  const batchCount = toNumber(readField(payload, ['BatchCount', 'batchCount', 'TotalBatches', 'totalBatches'])) ?? rows.length

  if (!rows.length && payloadTotal !== null) {
    rows.push({ key: 'harvest-summary', cropName: 'Tổng cộng', value: payloadTotal, unit })
  }
...
const normalizeAreaReport = (response, selectedCropLabel) => {
  const payload = getPayload(response)
  const sourceRows = getRows(response)
  const valueKeys = ['CultivatedArea', 'cultivatedArea', 'TotalArea', 'totalArea', 'AreaM2', 'areaM2', 'Area', 'area']
  const rows = sourceRows.map((row, index) => ({
    key: `area-${index}`,
  ...
  if (!rows.length && payloadTotal !== null) {
    rows.push({ key: 'area-summary', cropName: selectedCropLabel || 'Tổng cộng', value: payloadTotal, unit })
  }
  ...
  return {
    ...
    total,
    unit,
    cropName: selectedCropLabel || readField(payload, ['CropName', 'cropName', 'Crop', 'crop']) || 'Tất cả cây trồng',
  }
}

const normalizeMaterialReport = (response) => {
  const payload = getPayload(response)
  const valueKeys = ['QuantityUsed', 'quantityUsed', 'TotalQuantity', 'totalQuantity', 'Quantity', 'quantity']
  const rows = getRows(response).map((row, index) => ({
    key: `material-${index}`,
    name: readField(row, ['Material', 'material', 'MaterialName', 'materialName', 'Name', 'name']) || 'Vật tư',
    unit: readField(row, ['Unit', 'unit', 'QuantityUnit', 'quantityUnit']) || '—',
...
const ReportStatistics = () => {
  ...
  const fetchReport = useCallback(async (reportType = activeReport) => {
  ...
  const harvestColumns = [
    ...
    { title: 'Cây trồng', dataIndex: 'cropName', key: 'cropName', render: (value) => <Text strong>{value}</Text> },
    {
      title: 'Sản lượng thu hoạch',
      dataIndex: 'value',
      key: 'value',
      width: 220,
      render: (value, record) => <Text className="font-bold text-green-700">{formatUnitValue(value, record.unit)}</Text>,
    },
  ]
  ...
  const areaColumns = [
    ...
    { title: 'Cây trồng', dataIndex: 'cropName', key: 'cropName', render: (value) => <Text strong>{value}</Text> },
    {
      ...
      key: 'value',
      width: 220,
      render: (value, record) => <Text className="font-bold text-emerald-700">{formatUnitValue(value, record.unit)}</Text>,
    },
  ]

  const materialColumns = [
    { title: 'STT', width: 70, align: 'center', render: (_, __, index) => index + 1 },
    { title: 'Tên vật tư', dataIndex: 'name', key: 'name', render: (value) => <Text strong>{value}</Text> },
    {
      title: 'Tổng sử dụng',
  ...
  const columns = activeReport === REPORT_TYPES.HARVEST
    ? harvestColumns
    : activeReport === REPORT_TYPES.AREA ? areaColumns : materialColumns

  const emptyDescription = activeReport === REPORT_TYPES.HARVEST
    ...
    <div className="space-y-6 duration-500 animate-in fade-in slide-in-from-bottom-4">
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-end">
            {activeReport === REPORT_TYPES.AREA && (
              <div className="w-full lg:w-72">
                <Text className="mb-1 block text-sm font-medium">Cây trồng</Text>
                <Select
                  allowClear
                  showSearch
            ...
            <div className="w-full lg:w-auto">
              <Text className="mb-1 block text-sm font-medium">Thời gian từ A - B</Text>
              <RangePicker value={dateRange} format="DD/MM/YYYY" onChange={setDateRange} />
            </div>
            <Button type="primary" icon={<SearchOutlined />} onClick={() => fetchReport()} loading={reportLoading}>
              Xem báo cáo
            </Button>
          </div>
        </div>
      ...
      <Row gutter={[16, 16]}>
        <Col xs={24} md={12}>
          <Card bordered={false} className="rounded-2xl shadow-sm">
            <Statistic
              title={activeReport === REPORT_TYPES.HARVEST ? 'Tổng sản lượng thu hoạch' : activeReport === REPORT_TYPES.AREA ? 'Tổng diện tích canh tác' : 'Tổng vật tư đã sử dụng'}
        ...
        <Col xs={24} md={12}>
          <Card bordered={false} className="rounded-2xl shadow-sm">
            <Statistic
              title={activeReport === REPORT_TYPES.HARVEST ? 'Số lô thu hoạch' : activeReport === REPORT_TYPES.AREA ? 'Cây trồng đang xem' : 'Số loại vật tư'}
              value={activeReport === REPORT_TYPES.HARVEST ? reportData.batchCount ?? currentRows.length : activeReport === REPORT_TYPES.AREA ? reportData.cropName || 'Tất cả' : currentRows.length}
              suffix={activeReport === REPORT_TYPES.HARVEST ? 'lô' : activeReport === REPORT_TYPES.MATERIAL ? 'loại' : undefined}
