...
const CropProtectionFormModal = ({ open, editingItem, onClose, onSuccess }) => {
  ...
  return (
    <CustomModal
      ...
      title={
        <div className="flex items-center gap-2 py-1">
          ...
          <span className="font-bold">
            {isEdit ? 'Chỉnh sửa thuốc BVTV' : 'Thêm mới thuốc BVTV'}
          </span>
        </div>
      }
      footer={null}
      width={1000}
      destroyOnClose
      styles={{ body: { maxHeight: '78vh', overflowY: 'auto', overflowX: 'hidden', paddingRight: 8 } }}
    >
      <Form
        form={form}
        layout="vertical"
                ...
                <>
                  <div className="space-y-6 mb-3 mt-4">
                    {fields.map(({ key, name, ...restField }, index) => (
                      <div
                        key={key}
                        className="relative p-5 pt-7 bg-gray-50 border border-gray-200 rounded-2xl shadow-sm"
                      >
                        <div className="absolute -top-3 left-4 bg-emerald-100 text-emerald-700 px-3 py-1 rounded-full text-xs font-bold border border-emerald-200 shadow-sm shadow-emerald-50">
...
export default CropProtectionFormModal
