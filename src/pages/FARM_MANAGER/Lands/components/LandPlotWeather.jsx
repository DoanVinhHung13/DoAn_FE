...
const getWeatherIcon = (weather, className) => {
  ...
  if (
    ["113", "sunny", "clear"].includes(code.toLowerCase()) ||
...
const WeatherEmpty = ({ error, onRetry, compact }) => (
  <div
    className={`flex items-center gap-2 text-slate-400 ${compact ? "text-xs" : "text-sm"}`}
  >
    <AlertCircle className="h-4 w-4 shrink-0" />
    <span>
      {error ? "Không tải được thời tiết" : "Chưa có dữ liệu thời tiết"}
    </span>
    {error && onRetry && (
      <Button
        type="link"
...
const LandPlotWeather = ({
  ...
  if (!weather)
    return <WeatherEmpty error={error} onRetry={onRetry} compact={compact} />
      ...
      <div className="flex min-w-[155px] items-center gap-2">
        ...
        <div className="min-w-0">
          ...
          <Tooltip title={translateCondition(weather.condition)}>
            <div className="max-w-[115px] truncate text-xs text-slate-500">
  ...
  return (
    <Card
      ...
      title={
        <span className="font-semibold text-slate-800">Thời tiết hiện tại</span>
      }
      extra={
        onRetry && (
          <Button
        ...
        <div className="grid grid-cols-2 gap-3 sm:min-w-[300px]">
          <div className="rounded-xl bg-white/75 p-3">
            ...
            <div className="mt-1 font-semibold text-slate-800">
              {formatNumberWithUnit(weather.humidity, "%")}
            </div>
          </div>
          <div className="rounded-xl bg-white/75 p-3">
            <div className="flex items-center gap-1.5 text-xs text-slate-500">
