...
const typeLabel = value =>
  value === "FERTILIZER"
    ...
    : value === "PESTICIDE"
      ? "Nông dược"
      : "—"
const getLogbookName = row =>
  row.logbookName || row.cultivationLogbookName || row.logbook?.name || "—"
...
const MaterialUsageHistory = () => {
  const {
    ...
    initialFilters: { materialType: "all", dateRange: [] },
  })
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const result = unwrap(
        await MaterialUsageService.getHistory({
          PageIndex: page,
          PageSize: pageSize,
          ...
          SortDescending: timeSortOrder === "descend",
        }),
      )
      ...
      setRows(
        [...items].sort(
          (a, b) =>
            sortMultiplier *
            (getUsedAtTimestamp(a.usedAt) - getUsedAtTimestamp(b.usedAt)),
        ),
      )
      setTotal(
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 105,
      render: value => (
        <Tag className="m-0 whitespace-nowrap">{typeLabel(value)}</Tag>
      ),
    },
    {
      title: "Số lượng",
    ...
    },
    {
      ...
      render: (_, row) =>
        row.appliedArea != null
          ? `${row.appliedArea} ${formatAreaUnit(row.areaUnit || "m²")}`
          : "—",
    },
    {
      title: "Công việc",
    ...
    },
    {
      ...
      width: 190,
      ellipsis: true,
      render: (_, row) => getLogbookName(row),
    },
    {
  ...
  return (
    <div className={UI.page.wrapper}>
      ...
      <div className="admin-filter-card rounded-lg shadow-sm">
        <div className="admin-toolbar flex flex-col gap-3 xl:flex-row xl:items-center xl:flex-wrap">
          ...
          <div className="flex gap-2 xl:ml-auto">
            ...
            <Button
              aria-label="Tải lại lịch sử sử dụng vật tư"
              icon={<ReloadOutlined />}
              onClick={load}
              loading={loading}
      ...
      <CustomTable
        ...
        dataSource={rows}
        loading={loading}
        textEmpty="Chưa có lịch sử sử dụng vật tư."
