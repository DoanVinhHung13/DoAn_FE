...
const FarmSupplyCreate = () => {
  const navigate = useNavigate();
  const [form] = Form.useForm();

  const handleFinish = (values) => {
    message.success('Thêm vật tư nông nghiệp mới thành công!');
  ...
  return (
    <div className="space-y-6 duration-300 animate-in fade-in">
      <div className="flex items-center gap-3">
        ...
        <TitleCustom className="!mb-0">Thêm Vật tư Nông nghiệp mới</TitleCustom>
      </div>
      ...
      <Card className="rounded-2xl border-slate-200/80 shadow-xs w-full">
        <Form form={form} layout="vertical" onFinish={handleFinish} className="space-y-4">
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item name="category" label="Nhóm vật tư" rules={[{ required: true, message: 'Chọn nhóm vật tư' }]}>
                <Select options={CATEGORY_OPTIONS} placeholder="Chọn nhóm" className="rounded-xl" />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item name="unit" label="Đơn vị tính" initialValue="Cái" rules={[{ required: true, message: 'Nhập đơn vị tính' }]}>
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item name="stockQuantity" label="Số lượng tồn kho ban đầu" initialValue={10}>
                <InputNumber min={0} className="w-full rounded-xl" />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item name="minQuantity" label="Ngưỡng tồn kho tối thiểu (cảnh báo)" initialValue={5}>
                <InputNumber min={0} className="w-full rounded-xl" />
              </Form.Item>
            </Col>
          </Row>
...
export default FarmSupplyCreate;
