  ...
  const sidebarContent = (
    <div className="flex flex-col h-full bg-white">
      <div
        ...
        onClick={() => navigate(ROUTER.LM_DASHBOARD)}
      >
        {collapsed && !isMobile ? (
          ...
          <div className="flex items-center gap-4 w-full justify-center">
            ...
            <div className="flex flex-col text-center">
              <span className="text-blue-600 font-bold text-[13px] leading-[1.2]">LAND MANAGER</span>
    ...
    <Layout className="min-h-screen bg-[#f8fafc]">
      ...
      <Layout>
        ...
        >
          <Button
            type="text"
            icon={<MenuOutlined className="text-blue-600 text-xl" />}
            onClick={() => (isMobile ? setMobileMenuOpen(true) : setCollapsed(!collapsed))}
            className="w-10 h-10 flex items-center justify-center hover:bg-blue-50 rounded-xl transition-all"
          />
          ...
          <div className="flex items-center gap-2 md:gap-6">
            ...
            >
              <div className="flex items-center gap-2 md:gap-3 cursor-pointer group hover:bg-blue-50/50 p-1.5 md:pr-3 rounded-2xl transition-all border border-transparent hover:border-blue-100">
                <Avatar
                  size={isMobile ? 32 : 44}
                  src={getAvatarUrl(user?.avatar)}
                  className="bg-blue-50 text-blue-600 border-2 border-blue-200 group-hover:border-blue-400 transition-all font-bold shadow-sm"
                >
                  {!user?.avatar && getInitialAvatar(user?.fullname || user?.username || 'LM')}
                </Avatar>
                {!isMobile && (
                  <div className="text-left flex flex-col justify-center">
                    <Text className="font-bold text-gray-800 group-hover:text-blue-600 transition-colors block text-sm leading-tight">
                      {user?.fullname || user?.username || 'Land Manager'}
                    </Text>
...
export default LayoutLandManager
