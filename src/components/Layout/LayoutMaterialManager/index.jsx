...
const LayoutMaterialManager = () => {
  ...
  const menuItems = [
    ...
    },
    {
      ...
      label: <span className="font-medium">Quản lý phân bón</span>,
    },
    {
      key: ROUTER.MM_CROP_PROTECTIONS,
      icon: <ShieldCheck className="w-5 h-5" />,
      label: <span className="font-medium">Thuốc bảo vệ thực vật</span>,
    },
    {
      key: ROUTER.MM_MACHINERY,
      icon: <Tractor className="w-5 h-5" />,
      label: <span className="font-medium">Quản lý máy móc</span>,
    },
    {
      key: ROUTER.MM_OTHER_MATERIALS,
      icon: <Package className="w-5 h-5" />,
      label: <span className="font-medium">Vật tư khác</span>,
    },
    {
      key: ROUTER.MM_PURCHASE_REQS,
      icon: <ShoppingCart className="w-5 h-5" />,
      label: <span className="font-medium">Yêu cầu mua sắm</span>,
    },
    {
      key: ROUTER.MM_PRODUCTION_PLANS,
    ...
    },
    {
      ...
      label: <span className="font-medium">Theo dõi tác vụ</span>,
    },
  ]
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full bg-white">
      <div
        ...
        onClick={() => navigate(ROUTER.MM_DASHBOARD)}
      >
        {collapsed && !isMobile ? (
          ...
          <div className="flex items-center gap-4 w-full justify-center">
            ...
            <div className="flex flex-col text-center">
              <span className="text-orange-600 font-bold text-[13px] leading-[1.2]">MATERIAL MANAGER</span>
    ...
    <Layout className="min-h-screen bg-[#f8fafc]">
      ...
      <Layout>
        ...
        >
          <Button
            type="text"
            icon={<MenuOutlined className="text-orange-600 text-xl" />}
            onClick={() => (isMobile ? setMobileMenuOpen(true) : setCollapsed(!collapsed))}
            className="w-10 h-10 flex items-center justify-center hover:bg-orange-50 rounded-xl transition-all"
          />
          ...
          <div className="flex items-center gap-2 md:gap-6">
            ...
            >
              <div className="flex items-center gap-2 md:gap-3 cursor-pointer group hover:bg-orange-50/50 p-1.5 md:pr-3 rounded-2xl transition-all border border-transparent hover:border-orange-100">
                <Avatar
                  size={isMobile ? 32 : 44}
                  src={getAvatarUrl(user?.avatar)}
                  className="bg-orange-50 text-orange-600 border-2 border-orange-200 group-hover:border-orange-400 transition-all font-bold shadow-sm"
                >
                  {!user?.avatar && getInitialAvatar(user?.fullname || user?.username || 'MM')}
                </Avatar>
                {!isMobile && (
                  <div className="text-left flex flex-col justify-center">
                    <Text className="font-bold text-gray-800 group-hover:text-orange-600 transition-colors block text-sm leading-tight">
                      {user?.fullname || user?.username || 'Material Manager'}
                    </Text>
...
export default LayoutMaterialManager
