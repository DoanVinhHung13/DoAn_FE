...
;(function () {
  ...
  const outdentedComposer = (editor, entries) => {
    ...
    return map(normalizedEntries, entry => {
      ...
      return SugarElement.fromDom(
        createTextBlock(editor, content.dom, listItemAttrs),
      )
    })
  }
  ...
  const indentListSelection = editor =>
    handleIndentation(editor, "Indent" /* Indentation.Indent */)
  const outdentListSelection = editor =>
    handleIndentation(editor, "Outdent" /* Indentation.Outdent */)
  const flattenListSelection = editor =>
    handleIndentation(editor, "Flatten" /* Indentation.Flatten */)
    ...
    if (li) {
      ...
      if (otherLi && otherLi !== li && !willMergeParentIntoChild) {
        ...
        if (
          ...
          dom.isChildOf(otherLi, commonAncestorParent)
        ) {
          return false
        }
        editor.undoManager.transact(() => {
  ...
  const backspaceDeleteIntoListCaret = (editor, isForward) => {
    ...
    if (block && dom.isEmpty(block, undefined, { checkRootAsContent: true })) {
      ...
      const nextCaretContainer = findNextCaretContainer(
        editor,
        rng,
        isForward,
        root,
