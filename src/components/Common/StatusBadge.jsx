...
const StatusBadge = ({
  ...
  if (isActive !== undefined) {
    ...
    return (
      <span
        className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-[11px] font-bold cursor-default select-none ${bgColor} ${textColor} ${className}`}
      >
  ...
  return (
    ...
    >
      {showDot && <span className={`h-2 w-2 rounded-full ${config.dot}`} />}
      {displayLabel}
    </span>
  )
}
