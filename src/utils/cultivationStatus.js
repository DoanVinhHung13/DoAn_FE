...
const optionLabel = opt =>
  opt?.description || opt?.Description || opt?.label || opt?.Label || ""

const optionValue = opt =>
  opt?.codeValue ?? opt?.CodeValue ?? opt?.value ?? opt?.Value
...
export const resolveStatusMeta = (
  ...
  const found = systemKeyOptions.find(
    opt => String(optionValue(opt)) === String(status),
  )
    ...
    ...options
      ...
      .filter(
        opt =>
          opt.value != null &&
          opt.value !== "" &&
          !["CANCELLED", "CANCELED"].includes(String(opt.value).toUpperCase()),
      ),
  ]
}
...
export const canWriteDailyLog = status => status === "IN_PROGRESS"
