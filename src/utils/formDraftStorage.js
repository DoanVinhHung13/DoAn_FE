...
const SENSITIVE_FIELD_PATTERN = /(password|token|authorization|secret|otp|credential)/i
...
const serializeValue = (value, fieldName = '') => {
  if (SENSITIVE_FIELD_PATTERN.test(fieldName) || FILE_FIELD_PATTERN.test(fieldName)) return undefined
  ...
  if (Array.isArray(value)) {
    return value.map((item) => serializeValue(item, fieldName)).filter((item) => item !== undefined)
  }
...
export const loadFormDraft = (storageKey, { now = Date.now(), ttlMs = FORM_DRAFT_TTL_MS } = {}) => {
  ...
  try {
    ...
    if (payload?.version !== FORM_DRAFT_VERSION || !payload?.data || Number.isNaN(savedAt) || now - savedAt > ttlMs) {
