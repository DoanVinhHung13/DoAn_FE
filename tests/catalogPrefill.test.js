...
import {
  getApiData,
  getCatalogPrefill,
} from "../src/services/CatalogSuggestionService/response.js"
import {
...
test("unwraps ApiResponse data to a fertilizer prefill object", () => {
  const prefill = {
    name: "AC-Amino Bo",
    compositions: [{ name: "N", value: 3, unit: "%" }],
  }
  assert.deepEqual(getCatalogPrefill({ success: true, data: prefill }), prefill)
  assert.deepEqual(
    getCatalogPrefill({ data: { success: true, data: prefill } }),
    prefill,
  )
  assert.deepEqual(getApiData({ success: true, data: [{ id: "1" }] }), [
    { id: "1" },
  ])
})
...
test("preserves the complete AN-OGR composition list", () => {
  ...
  assert.equal(
    normalizeFertilizerCatalogCompositions(source).length,
    source.length,
  )
})
...
test("maps catalog compositions to keyed rows without placeholders", () => {
  ...
  assert.deepEqual(
    rows.map(({ name, value, unit }) => ({ name, value, unit })),
    [
      { name: "HC", value: 15, unit: "%" },
      { name: "N", value: 6, unit: "%" },
      { name: "P2O5", value: 3, unit: "%" },
